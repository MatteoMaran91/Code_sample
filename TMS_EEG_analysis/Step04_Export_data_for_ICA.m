%%  In Step04
%% IN THE PREVIOUS STEP I FORGOT TO EXPORT DATA_FOR_ICA. I re-run the script without calculating the ICA components
        ...
        for bi = 1:length(my_blocks)
            ...
            if error_count == 0
                ...
                if error_count == 0
                    ...
                    %% EXPORT DATA FOR ICA
                    data_for_ica_filename = [subjectdata.output_fold, 'Sub', sprintf('%02d',c_subj), '_', c_tms, '_B', num2str(c_blk),'_data_for_ica.mat'];
                    save(data_for_ica_filename, 'data_for_ica')

                    %% DISPLAY STATUS
                    disp(['####################### DATA FOR ICA FOR SUBJ ',  sprintf('%02d',c_subj), ' ', c_tms, ' BLOCK ', num2str(c_blk), ' EXPORTED #######################']);
...
publish([folder_pdf,'Step04_Export_data_for_ICA.m'],'format','pdf', 'evalCode',false)
...
cur_script_name = 'Step04_Export_data_for_ICA';
